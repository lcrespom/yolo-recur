---
name: create-route
description: Create a new TanStack Router page with proper data fetching, loading/error states, and DaisyUI styling
...
Create a new route page at `src/routes/$ARGUMENTS.tsx` following the project's TanStack Router file-based routing conventions.

The argument is the route path (e.g., `settings/index` for `/settings`, or `payments/$paymentId` for a dynamic route).

## Requirements

1. **Read existing routes first**: Before writing any code, read at least two existing route files in `src/routes/` to understand the project's patterns
2. **Never edit** `src/routeTree.gen.ts` — it is auto-generated by TanStack Router
...
7. **Loading state**: Render a `<span className="loading loading-spinner loading-lg"></span>` centered
...
1. Read existing route files for reference (especially `src/routes/dashboard/index.tsx` and `src/routes/payments/$paymentId.tsx`)
