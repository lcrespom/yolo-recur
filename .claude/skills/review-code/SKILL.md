...
Perform a thorough code review of `$ARGUMENTS` (a file path or directory). If no argument
is provided, review recent changes or the full `src/` directory.

## Review Checklist

### 1. Code Conventions

- [ ] **Avoid repetition**: no duplicate code blocks, consider abstraction if patterns
      repeat.
- [ ] **Avoid too large functions/components**: if a function or component is too long,
      consider breaking it down into smaller pieces.
- [ ] **No semicolons**, single quotes, trailing commas (es5), 90-char print width
- [ ] **No empty lines inside functions/components** — only between them.
...
- [ ] **Data fetching pattern**: uses the project's useState + useCallback + useEffect
      pattern
...
- [ ] **Responsive design**: mobile-first approach, uses responsive breakpoints (sm:, md:,
      lg:)
- [ ] **Dark/light theme**: uses DaisyUI semantic colors (base-100, primary, error), not
      raw colors
- [ ] **Currency formatting**: uses `config.currencySymbol`

### 4. TypeScript Quality

- [ ] **Types defined for props** — placed above the component
...
- [ ] **Semantic HTML**: buttons for actions, links for navigation, proper heading
      hierarchy
...
- [ ] **No unnecessary fetches**: data loaded once and passed down, not re-fetched in
      children
