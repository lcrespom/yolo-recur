...
Migrate the service at `src/services/$ARGUMENTS.ts` from json-server `fetch()` calls to Supabase client API calls, preserving the same exported function signatures.

## Pre-Migration Checklist

1. **Read the existing service file** to understand all exported functions and their signatures
...
4. **Check if Supabase client is already set up** — look for `src/lib/supabase.ts` or similar
...
Create helper functions `toSnakeCase()` and `toCamelCase()` or a mapper if many fields need conversion between the API and TypeScript interfaces.

## Important Notes

- **Preserve the exact same function signatures** — all calling code should work without changes
- **Use the Context7 MCP tools** to look up current Supabase JS client documentation if unsure about query syntax
- **RLS is active** — queries will be scoped to the authenticated user automatically. Ensure the user is logged in before making service calls
- **IDs**: Supabase uses UUID by default. The existing `id: string` type is compatible
- **Error handling**: Map Supabase `error` objects to thrown Error instances to match the existing pattern
