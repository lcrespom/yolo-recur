...
model: haiku
allowed-tools:
  Read, Glob, Grep, Edit, Write, Bash(npx tsc:*), Bash(npm run lint), Bash(npm test),
  Bash(npx vitest:*), mcp__vscode__code_checker
---

# Find and Fix Errors

Systematically find and fix all errors in the project. Optionally target a specific area:
`$ARGUMENTS` (e.g., `types`, `lint`, `tests`, or a file path).
